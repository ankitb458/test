...
function get_theme_features()
{
	return array(
		...
		'WordPress Tweaks and Fixes' => array(
			'autolink_uri' => array(
					'Autolink uri',
			...
			'highlight_search' => array(
					'Highlight Search',
					'Highlight the searched for strings in search result listings.'
					),
		...
		'WordPress Features' => array(
			...
			'exec_php' => array(
					'Execute php',
					'Allow php in Pages and in sidebars via php Widgets.'
					),
			),
		...
		'Extra Sidebar Widgets' => array(
			...
			'newsletter_manager' => array(
					...
					'It will work with any list manager that can answer a list-subscribe@domain.com query.'
					),
			'paypal_donate' => array(
					'Paypal Donate',
		...
		'Permalink Features' => array(
			'404_handler' => array(
					'404 Handler',
					'Monitor and log 404 errors on your site. Configure via Options / 404 Handler'
					),
			...
			'kill_index_php' => array(
					'Enforce no index.php',
					'Enforce no index.php. Do not use if you are using a permalink structure with index.php in it.'
					),
		...
		'Search Engine Optimization' => array(
			...
			'theme_meta' => array(
					'Automatic Meta Tags',
					'Automatically generate meta keywords (post categories) and description (psot excerpt) tags.'
					),
			'theme_title' => array(
		...
		'Site Statistics' => array(
			'feedburner' => array(
					'FeedBurner',
					'Statistics for your RSS feed, via <a href="http://www.feedburner.com">Feedburner</a>. Configure via Options / FeedBurner.'
					),
		...
		'Promote Semiologic' => array(
			...
			'theme_credits' => array(
					'Theme Credits',
					'Give credits to the theme\'s and the skin\'s authors.'
					),
			),
		...
		'Site Management' => array(
			'admin_menu'
				=> array(
					...
					'Adds an admin menu to my blog, for convenient access to the most commonly accessed admin areas.'
					),
			'db_backup' => array(
		...
		'Cron Jobs' => array(
			...
			'cron_email' => array(
					...
					'Periodically check the site\'s mailbox, assuming you configured the post by email options (Options / Writing).'
					),
			'cron_gravatar' => array(
					'Refresh Gravatar Cache',
					'Periodically refresh the Gravatar Cache.'
					),
			'cron_links' => array(
...
<?php
	...
	foreach ( $all_features as $feature_set_name => $feature_set )
	{
		...
		foreach ( array_keys($feature_set) as $feature_id )
		{
...
<h3><label for="all_<?php echo str_replace(',', '_', $feature_ids); ?>"><input type="checkbox"
		...
		<?php if ( function_exists('theme_feature_is_active') ) : ?>
		onchange="if ( this.checked ) check_all('<?php echo $feature_ids; ?>'); else uncheck_all('<?php echo $feature_ids; ?>');"
		<?php else : ?>
		disabled="disabled"
		<?php endif; ?>
		/>
...
?>
	<p>
		...
		<input type="checkbox"
			...
			<?php if ( !function_exists('theme_feature_is_active') ) : ?>
			disabled="disabled"
			<?php elseif ( function_exists('theme_feature_is_active')
					&&
					isset($feature_description[2])
					) : ?>
			disabled="disabled"
			checked="checked"
			<?php elseif ( theme_feature_is_active($feature_id)
					||
					isset($feature_description[2])
					) : ?>
			checked="checked"
			...
			<?php echo $feature_description[1]; ?>
			<?php else : ?>
			<strong><?php echo $feature_description[0]; ?></strong><br />
			<?php echo $feature_description[1]; ?>
			<?php endif; ?>
			</label>
	</p>
