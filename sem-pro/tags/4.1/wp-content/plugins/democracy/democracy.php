...
?>
	       ...
	       <td style="text-align: center">
	           <form action="" method="get">
	               <div>
	                   ...
	                   <?php if ($current !== $r->id) :  ?>
	                       <input type="submit" value="Activate" name="jal_dem_activate" />
	                   <?php else : ?>
	                       <input type="submit" value="Deactivate" name="jal_dem_deactivate" />
	                   <?php endif; ?>
    ...
    <p>Polls are fully HTML compatible. No character entities will be converted, so if you want to use &amp;, write <code>&amp;amp;</code>, etc.. If you have no idea know what the last two sentences meant, don't worry about 'em. Blank fields will be skipped.</p>
    <form action="edit.php?page=democracy" method="post" onsubmit="return jal_validate();">
 ...
 if ($_GET['jal_no_js'] || isset($_COOKIE['demVoted_'.$poll_question->id]))
    ...
    ?>
     <form action="<?php echo get_bloginfo('wpurl') . '/wp-content/plugins/democracy/democracy.php?jal_nojs=true'; ?>" method="post" id="democracyForm" onsubmit="return ReadVote();">
        ...
        <ul>
    	  <?php foreach ( $poll_answers as $r) { ?>
            <li>
            	<label for="choice_<?php echo $r->aid; ?>">
            		...
            		<?php echo stripslashes($r->answers); ?><?php if ($r->added_by == "1") { echo '<sup title="Added by users">1</sup>'; $user_added = TRUE; }?>
            	</label>
            </li>
    	  <?php }
    	  if ( $poll_question->allowusers == 1) {
    	   ?>
			...
			<?php } else { ?>
    	    <li>
    	    	<a href="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']).$x; ?>jal_add_user_answer=true" id="jalAddAnswer">Add an Answer</a>
        ...
        ?>
        	<a id="view-results" href="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']).$x; ?>jal_no_js=true&amp;poll_id=<?php echo $poll_question->id; ?>">View Results</a>
        	<?php if ($user_added) echo "<br /><small><sup>1</sup> = Added by a guest</small>"; ?>
        <?php } else { echo "No votes yet"; } ?></p>
       </div>
    </form>
        ...
        // In the graphs, define which class/id to use.
         ...
         <?php /* if they are just looking at the results and haven't voted */
          if (!$_COOKIE['demVoted_'.$poll_question->id]) { ?>
                                        <br />
                                        <a href="<?php echo htmlspecialchars($_SERVER['HTTP_REFERER']); ?>">Vote</a>
     <?php } ?>
...
if (isset($_GET['jal_nojs'])) {
    ...
    if($_POST['poll_aid'] == "newAnswer") {
       ...
       setcookie("demVoted_".$_POST['poll_id'],$aid + 1,time()+$jal_cookietime, $jal_cookie_path);
	} else
	    setcookie("demVoted_".$_POST['poll_id'],$_POST['poll_aid'],time()+$jal_cookietime, $jal_cookie_path);
...
if (isset($_GET['demSend'])) {
	...
	if (isset($_POST['new_vote'])) {
		...
		if ($wpdb->get_var("SELECT allowusers FROM {$table_prefix}democracyQ WHERE id = '{$poll_id}'") == 1) {
    		...
    		$wpdb->query("INSERT INTO {$table_prefix}democracyA (qid, answers, votes, added_by) VALUES ($poll_id, '".$wpdb->escape($_POST['vote'])."', 1, 1)");
    	    // Get the new id of the new answer
    	}
	} else
		$wpdb->query("UPDATE {$table_prefix}democracyA SET votes = (votes+1) WHERE qid = {$poll_id} AND aid = ".$wpdb->escape($_POST['vote']));
}
...
function widget_democracy_init()
{
	...
	function widget_democracy_control()
	{
		...
		if ($_POST['democracy-submit']) {
			$options['title'] = strip_tags(stripslashes($_POST['democracy-title']));
