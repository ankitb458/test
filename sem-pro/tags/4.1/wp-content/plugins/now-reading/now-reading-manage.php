	...
	switch ( $action ) {
		case 'editsingle':
			...
			<div class="wrap">
				...
				<form method="post" action="' . get_settings('siteurl') . '/wp-content/plugins/now-reading/edit.php">
			';
			...
			foreach ( (array) $meta as $key => $val ) {
				$url = get_settings('siteurl') . "/wp-content/plugins/now-reading/edit.php?action=deletemeta&id={$existing->id}&key=" . urlencode($key);
			...
			$delete = get_settings('siteurl') . '/wp-content/plugins/now-reading/edit.php?action=delete&id=' . $existing->id;
			if ( function_exists('wp_nonce_url') )
	...
	if ( $list ) {
		...
		if ( $count ) {
			...
			foreach ( (array) $books as $book ) {
				...
				$delete = get_settings('siteurl') . '/wp-content/plugins/now-reading/edit.php?action=delete&id=' . $book->id;
				if ( function_exists('wp_nonce_url') )
					$delete = wp_nonce_url($delete, 'now-reading-delete-book_' . $book->id);
