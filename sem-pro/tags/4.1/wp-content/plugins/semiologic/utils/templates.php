...
function setup_advanced_template($template)
{
	switch ($template)
	{
		...
		case 'sell_page':
			...
			add_filter('show_entry_comments', 'false');
	...
	$cur_posts = $wpdb->get_results("
		SELECT
			...
			. ( highlights_cat_id
				? ( ",
			MAX( CASE
				WHEN post2cat.category_id = " . highlights_cat_id . "
					THEN 1
					ELSE 0
