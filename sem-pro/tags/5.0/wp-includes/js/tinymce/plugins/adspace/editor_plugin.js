...
function TinyMCE_adspace_getControlHTML(control_name)
{
	switch (control_name) {
		case "adspace":
			var html = '<select id="{$editor_id}_ad_unit_select" name="{$editor_id}_ad_unit_select" onchange="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mce_ad_unit_select\',false,this.options[this.selectedIndex].value);this.selectedIndex=0;" class="mceSelectList" style="font-size:8pt;">';
...
function TinyMCE_adspace_make_imgtag(ad_name){
	var html = ''
			+ '<img src="' + (tinyMCE.baseURL + "/plugins/adspace/images/ad-unit.gif") + '" '
			+ ' width="110" height="30" '
...
function TinyMCE_adspace_cleanup(type, content) {
	switch (type) {
		...
		case "insert_to_editor":
			...
			// Parse all <!--ad_unit--> tags and replace them with images
		...
		case "get_from_editor":
			// Parse all img tags and replace them with <!--ad_unit-->
			var startPos = -1;
