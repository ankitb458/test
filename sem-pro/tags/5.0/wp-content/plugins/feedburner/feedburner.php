<?php
/*
Plugin Name: FeedBurner FeedSmith (fork)
...
Update Tag: feedburner
Update URI: http://www.semiologic.com/members/sem-pro/download/

Security enhancements by Denis de Bernardy <http://www.getsemiologic.com>
*/
...
function ol_feedburner_options_subpanel() {
	...
	{
		if (ol_is_authorized() ) {
			check_admin_referer('feedburner');
			if (isset($_POST['feedburner_url'])) {
				$feedburner_settings['feedburner_url'] = stripslashes(strip_tags($_POST['feedburner_url']));
			}
			if (isset($_POST['feedburner_comments_url'])) {
				$feedburner_settings['feedburner_comments_url'] = stripslashes(strip_tags($_POST['feedburner_comments_url']));
			}
	...
	if (ol_is_authorized()) {
		...
		if ( function_exists('wp_nonce_field') ) wp_nonce_field('feedburner');
