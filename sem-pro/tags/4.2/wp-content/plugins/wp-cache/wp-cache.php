...
$wp_cache_config_file_sample = ABSPATH . 'wp-content/'
		. ( function_exists('get_site_option')
			? 'mu-plugins/'
			: 'plugins/'
			)
		. 'wp-cache/wp-cache-config-sample.php';
$wp_cache_link = ABSPATH . 'wp-content/advanced-cache.php';
$wp_cache_file = ABSPATH . 'wp-content/'
		. ( function_exists('get_site_option')
			? 'mu-plugins/'
			: 'plugins/'
			)
...
if ( !defined('cache_mu') )
{
	define('cache_mu', true);

	if ( is_writable($cache_path) )
	{
		$cache_path = $cache_path . intval($blog_id);
...
function wp_cache_add_pages() {
	...
	{
		add_options_page('WP-Cache Manager', 'WP-Cache', 7, __FILE__, 'wp_cache_manager');
	}
}
		...
		while ( false !== ($file = readdir($handle))) {
			if ( preg_match("/^$file_prefix.*\.meta/", $file) ) {
				...
				if ($list_files) {
					...
					$flip = !$flip;
					echo '<td><a href="http://'. $_SERVER["HTTP_HOST"] . $meta->uri . '" target="_blank" >';
					echo $meta->uri . "</a></td>";
