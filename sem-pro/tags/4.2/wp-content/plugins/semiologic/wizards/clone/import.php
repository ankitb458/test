...
function get_semiologic_config($errors, $site_uri, $user_login, $user_pass)
{
	foreach ( array('version', 'user', 'options', 'ads') as $data )
	{
		...
		if ( $$data === false )
		{
			$errors[] = 'Failed to open stream: Please double check the cloned site\'s details';
			break;
		}
		...
		if ( $data == 'version' )
		{
			...
			{
				$errors[] = 'Failed to validate version: Please make sure both ends are using the same clone script version, and upgrade one or both sites as necessary';
				break;
			}
		}
		else
...
function import_semiologic_config($user, $options, $ads)
{
	...
	# ads
	foreach ( $ads as $table_name => $table_data )
	{
