...
function widget_gsearch_init() {
	...
	function widget_gsearch_control() {
		...
		$options = get_option('widget_gsearch');
		if ( !is_array($options) )
			$options = array('title'=>'', 'buttontext'=>'Google Search');
		if ( $_POST['gsearch-submit'] ) {

			// Remember to sanitize and format use input appropriately.
			$options['title'] = wp_filter_post_kses(strip_tags(stripslashes($_POST['gsearch-title'])));
			$options['buttontext'] = stripslashes(wp_filter_post_kses(strip_tags($_POST['gsearch-buttontext'])));
		...
		echo '<p style="text-align:right;"><label for="gsearch-title">Title: <input style="width: 200px;" id="gsearch-title" name="gsearch-title" type="text" value="'.$title.'" /></label></p>';
		echo '<p style="text-align:right;"><label for="gsearch-buttontext">Button Text: <input style="width: 200px;" id="gsearch-buttontext" name="gsearch-buttontext" type="text" value="'.$buttontext.'" /></label></p>';
		echo '<input type="hidden" id="gsearch-submit" name="gsearch-submit" value="1" />';
	}

	// This registers our widget so it appears with the other available
	// widgets and can be dragged and dropped into any active sidebars.
	register_sidebar_widget('Google Search', 'widget_gsearch');

	// This registers our optional widget control form. Because of this
	// our widget will have a button that reveals a 300x100 pixel form.
	register_widget_control('Google Search', 'widget_gsearch_control', 300, 100);
}

// Run our code later in case this loads prior to any required plugins.
add_action('plugins_loaded', 'widget_gsearch_init');
