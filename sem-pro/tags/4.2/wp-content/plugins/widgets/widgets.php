...
function sidebar_admin_head() {
	...
	* html .placematt {
	...
	<script type="text/javascript">
		// <![CDATA[
		var cols = new Array;
<?php $i = 0; foreach ( array_merge(array('palette'=>array()), $registered_sidebars) as $index => $sidebar ) : ?>			cols[<?php echo $i++; ?>] = '<?php echo $index; ?>';
<?php endforeach; ?>		var widgets = new Array;
<?php $i = 0; foreach ( $registered_widgets as $name => $widget ) : $san_name = sanitize_title($name); ?>			widgets[<?php echo $i++; ?>] = '<?php echo $san_name; ?>';
<?php endforeach; ?>		var controldims = new Array;
<?php foreach ( $registered_widget_controls as $name => $control ) : ?>			controldims['<?php echo sanitize_title($name); ?>control'] = new Array;
...
<?php endforeach; ?>		function initWidgets() {
<?php foreach ( $registered_widget_controls as $name => $control ) : $san_name = sanitize_title($name); ?>			$('<?php echo $san_name; ?>popper').onclick = function() {popControl('<?php echo $san_name; ?>control');};
...
<?php endforeach; ?>			if ( true && window.opera )
...
<?php foreach ( $registered_sidebars as $index => $sidebar ) : ?>			$('<?php echo $index; ?>order').value = Sortable.serialize('<?php echo $index; ?>');
<?php endforeach; ?>		}
...
<?php
	...
	?>	<?php if ( $_POST['action'] ) { ?>	<div class="fade updated" id="message">
...
<?php $i = 1; foreach ( $registered_sidebars as $index => $sidebar ) : ?>	<input type="hidden" id="<?php echo $index; ?>order" name="<?php echo $index; ?>order" value="" />
	<div class="dropzone">
...
<?php endforeach; ?>	<br class="clear" />
	...
	<script type="text/javascript">
	// <![CDATA[
<?php foreach ( $containers as $container ) : ?>	Sortable.create("<?php echo $container; ?>",
...
<?php endforeach; ?>	// ]]>
	...
	<div id="controls">
<?php foreach ( $registered_widget_controls as $name => $control ) : ?>		<div class="hidden" id="<?php echo sanitize_title($name); ?>control">
			<span class="controlhandle"><?php echo $name; ?></span>
			<span id="<?php echo sanitize_title($name); ?>closer" class="controlcloser">&#215;</span>
			<div class="controlform">
<?php call_user_func_array($control['callback'], $control['params']); ?>			</div>
		</div>
<?php endforeach; ?>	</div>
	</form>
	<br class="clear" />
...
?>	<div class="wrap">
		<form method="POST">
			<h2>Text Widgets</h2>
			<p style="line-height: 30px;"><?php _e('How many text widgets would you like?'); ?>			<select id="text-number" name="text-number" value="<?php echo $options['number']; ?>">
<?php for ( $i = 1; $i <= 20; $i++ ) echo "<option value='$i' ".($options['number']==$i ? "selected='selected'" : '').">$i</option>"; ?>			</select>
...
function widget_text_register() {
	...
	for ($i = 1; $i <= 20; $i++) {
		...
		register_widget_control($name, $i <= $number ? 'widget_text_control' : /* unregister */ '', 460, 400, $i);
	}
...
?>		<?php echo $before_widget; ?>			<?php echo $before_title . $title . $after_title; ?>			<ul>
			<?php wp_list_cats("sort_column=name&optioncount=$c&hierarchical=$h&hide_empty=1"); ?>			</ul>
		<?php echo $after_widget; ?><?php
}
...
function widget_categories_control() {
	...
	$hierarchical = ( !isset($options['hierarchical']) || $options['hierarchical'] ) ? 'checked="checked"' : '';
...
function widget_recent_entries($args) {
	...
	if ($r->have_posts()) :
?>		<?php echo $before_widget; ?>			<?php echo $before_title . $title . $after_title; ?>			<ul>
			<?php  while ($r->have_posts()) : $r->the_post(); ?>			<li><a href="<?php the_permalink() ?>"><?php if ( get_the_title() ) the_title(); else the_ID(); ?> </a></li>
			<?php endwhile; ?>			</ul>
		<?php echo $after_widget; ?><?php
	endif;
}
...
?>
		<?php echo $before_widget; ?>			<?php echo $before_title . $title . $after_title; ?>			<ul id="recentcomments"><?php
...
function widget_rss($args, $number = 1) {
	...
	if ( file_exists(dirname(__FILE__) . '/rss.png') )
		$icon = str_replace(str_replace('\\', '/', ABSPATH), get_settings('siteurl').'/', str_replace('\\', '/', dirname(__FILE__))) . '/rss.png';
	else
...
?>		<?php echo $before_widget; ?>			<?php $title ? print($before_title . $title . $after_title) : null; ?>			<ul>
<?php
	if ( $rss ) {
...
?>	<div class="wrap">
		<form method="POST">
			...
			<p style="line-height: 30px;"><?php _e('How many RSS widgets would you like?'); ?>			<select id="rss-number" name="rss-number" value="<?php echo $options['number']; ?>">
<?php for ( $i = 1; $i <= 20; $i++ ) echo "<option value='$i' ".($options['number']==$i ? "selected='selected'" : '').">$i</option>"; ?>			</select>
...
register_widget_control('Pages', 'widget_pages_control', 300, 150);
register_sidebar_widget('Calendar', 'widget_calendar');
register_widget_control('Calendar', 'widget_calendar_control', 300, 90);
