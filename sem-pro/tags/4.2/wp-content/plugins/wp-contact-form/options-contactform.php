...
if ('process' == $_POST['stage'])
{
	...
	#var_dump($_POST);
	#echo '</pre>';
update_option('wpcf_email', stripslashes(wp_filter_post_kses(strip_tags($_POST['wpcf_email']))));
update_option('wpcf_request_phone', intval((isset($_POST['wpcf_request_phone']) || isset($_POST['wpcf_require_phone']))));
update_option('wpcf_require_phone', intval(isset($_POST['wpcf_require_phone'])));
update_option('wpcf_subject_suffix', stripslashes(wp_filter_post_kses(strip_tags($_POST['wpcf_subject_suffix']))));
update_option('wpcf_success_msg', stripslashes(wp_filter_post_kses($_POST['wpcf_success_msg'])));
update_option('wpcf_error_msg', stripslashes(wp_filter_post_kses($_POST['wpcf_error_msg'])));
update_option('wpcf_textarea_cols', 30);
update_option('wpcf_textarea_rows', 8);
	...
	<fieldset class="options">
		...
		<table width="100%" cellspacing="2" cellpadding="5" class="editform">
		  <tr valign="top">
			...
			<br />
	<?php _e('Upon successful form submission, this is the message the user will see.<br />Some HTML allowed.', 'wpcf') ?></td>
		  </tr>
		  <tr valign="top">
			...
			<br />
	<?php _e('If the user skips a required field (or enters an invalid email address), this message will be displayed.<br />Some HTML allowed.', 'wpcf') ?>		  </tr>
		</table>
...
<br />
<?php _e('This setting allows you to adjust the width (columns) of the contact form message box.', 'wpcf') ?>            </td>
          </tr>
        <tr valign="top">
...
<br />
<?php _e('This setting allows you to adjust the height (rows) of the contact form message box.', 'wpcf') ?>            </td>
          </tr>
<?php endif; ?>          <tr valign="top">
            <th width="30%" scope="row" style="text-align: left"><?php _e('Request Phone Number', 'wpcf') ?></th>
            <td>
                <label for="wpcf_request_phone"><input type="checkbox" id="wpcf_request_phone" name="wpcf_request_phone"
            <?php if(get_option('wpcf_request_phone') || get_option('wpcf_require_phone')) {?> checked="checked" <?php } ?> /> <?php _e('Request a phone number when messages are sent to you.', 'wpcf') ?></label>
            </td>
          </tr>
          <tr valign="top">
            <th width="30%" scope="row" style="text-align: left"><?php _e('Require Phone Number', 'wpcf') ?></th>
            <td>
                <label for="wpcf_require_phone"><input type="checkbox" id="wpcf_require_phone" name="wpcf_require_phone"
            <?php if(get_option('wpcf_require_phone') ) {?> checked="checked" <?php } ?> /> <?php _e('Require a phone number when messages are sent to you.', 'wpcf') ?></label>
            </td>
          </tr>
<?php if (false) : ?>          <tr valign="top">
	      ...
	      <tr valign="top">
	        ...
	        <td>
	        	...
	        	<?php if($wpcf_show_quicktag == TRUE) {?> checked="checked" <?php } ?> /><br /><?php _e('This setting will enable the \'Contact Form\' Quicktag in the post/page editor', 'wpcf') ?>			</td>
	      </tr>
<?php endif; ?>	     </table>
