...
class RoleManager
{
	...
	function rename_role_form($role) {
		global $wp_roles;?>		<div class="wrap">
			...
			<form method="post" class="role-manager" action="<?php echo $this->manage_roles_uri(); ?>">
				...
				<label for="role-name"><?php _e('New Name:', 'role-manager'); ?>					<input type="text" name="role-name" id="role-name"
	...
	function manage_roles_page()
	{
		...
		<form method="post" class="role-manager">
			...
			<fieldset>
				<?php foreach ($this->get_cap_list(true) as $cap) { ?>					<label for="cap-<?php echo $cap; ?>" class="cap-label">
