...
/*
Plugin Name: Comment Status Manager
...
$auto_check = is_numeric(wp_next_scheduled('eco_auto'));
...
elseif ( $eco_clean['for'] == 'for-comments' ) {
	...
	if (is_numeric($_POST['number-comments'])) {
	...
	*/
	...
	if (true == $eco_clean['auto-last'] || true == $auto_check) {
		...
		if ('posts' == $eco_clean['units']) {
			...
			if (false == is_numeric(wp_next_scheduled('eco_auto'))) {
				wp_clear_scheduled_hook('eco_auto');
