	...
	function widgetize()
	{
		...
		if ( !( $options = get_option('tag_cloud_widgets') ) )
		{
			...
			foreach ( array_keys( (array) $sidebars = get_option('sidebars_widgets') ) as $k )
			{
				...
				elseif ( ( $key = array_search('Tag_Cloud', $sidebars[$k]) ) !== false )
				{
					$options = array( 1 => array() );
					$sidebars[$k][$key] = 'tag_cloud_widget-1';
					update_option('sidebars_widgets', $sidebars);
		...
		foreach ( array_keys($options) as $o )
		{
			...
			wp_register_sidebar_widget($id, __('Tag Cloud Widget'), array('tag_cloud_widgets', 'display_widget'), $widget_options, array( 'number' => $o ));
			wp_register_widget_control($id, __('Tag Cloud Widget'), array('tag_cloud_widgets_admin', 'widget_control'), $control_options, array( 'number' => $o ) );
		}
		...
		if ( !$id )
		{
			...
			wp_register_sidebar_widget($id, __('Tag Cloud Widgets'), array('tag_cloud_widgets', 'display_widget'), $widget_options, array( 'number' => -1 ));
			wp_register_widget_control($id, __('Tag Cloud Widgets'), array('tag_cloud_widgets_admin', 'widget_control'), $control_options, array( 'number' => -1 ) );
		}
	} # widgetize()
...
if ( is_admin() )
{
	include dirname(__FILE__) . '/tag-cloud-admin.php';
}
?>