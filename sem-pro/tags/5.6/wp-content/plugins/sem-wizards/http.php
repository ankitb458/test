...
class sem_http
{
	...
	function get($url)
	{
		...
		else
		{
			...
			$_SESSION['sem_err'][] = '<div class="error">'
				. '<p>'
				. '<b>' . sprintf(__('Cannot access %s'), preg_replace("/\?.*/", '', $url)) . '</b>' . '<br />'
				. __('allow_url_fopen is turned off, and an open_basedir limitation prevents CURLOPT_FOLLOWLOCATION to work. Please contact your host to resolve this issue by having them toggle either setting (or consider <a href="http://www.semiologic.com/resources/wp-basics/wordpress-hosts/">changing hosts</a>).')
				. '</p>'
				. '</div>';
