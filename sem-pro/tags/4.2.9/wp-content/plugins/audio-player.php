...
function add_ap_option($name, $value, $description, $autoload)
{
...
function update_ap_option($name, $value)
{
...
// Option defaults
add_ap_option('audio_player_web_path', '/audio', "Web path to audio files", true);
add_ap_option('audio_player_behaviour', 'default', "Plugin behaviour", true);
add_ap_option('audio_player_rssalternate', 'nothing', "RSS alternate content", true);
add_ap_option('audio_player_rsscustomalternate', '[See post to listen to audio]', "Custom RSS alternate content", true);
add_ap_option('audio_player_prefixaudio', '', "Pre-Stream Audio", true);
add_ap_option('audio_player_postfixaudio', '', "Post-Stream Audio", true);

if(get_ap_option('audio_player_iconcolor') != '' && get_ap_option('audio_player_lefticoncolor') == '') {
	// Upgrade options from version 0.x
	$ap_color = '';
	$ap_color = str_replace("#", "0x", get_ap_option('audio_player_iconcolor'));
	add_ap_option('audio_player_lefticoncolor', $ap_color, "Left icon color", true);
	add_ap_option('audio_player_righticoncolor', $ap_color, "Right icon color", true);
	add_ap_option('audio_player_righticonhovercolor', $ap_color, "Right icon hover color", true);
	delete_option('audio_player_iconcolor');

	update_ap_option('audio_player_textcolor', str_replace("#", "0x", get_ap_option('audio_player_textcolor')));

	$ap_color = str_replace("#", "0x", get_ap_option('audio_player_bgcolor'));
	update_ap_option('audio_player_bgcolor', $ap_color);
	add_ap_option('audio_player_leftbgcolor', $ap_color, "Left background color", true);

	$ap_color = str_replace("#", "0x", get_ap_option('audio_player_buttoncolor'));
	add_ap_option('audio_player_rightbgcolor', $ap_color, "Right background color", true);
	delete_option('audio_player_buttoncolor');

	$ap_color = str_replace("#", "0x", get_ap_option('audio_player_buttonhovercolor'));
	add_ap_option('audio_player_rightbghovercolor', $ap_color, "Right background hover color", true);
	delete_option('audio_player_buttonhovercolor');

	$ap_color = str_replace("#", "0x", get_ap_option('audio_player_pathcolor'));
	add_ap_option('audio_player_trackcolor', $ap_color, "Progress track color", true);
	delete_option('audio_player_pathcolor');

	$ap_color = str_replace("#", "0x", get_ap_option('audio_player_barcolor'));
	add_ap_option('audio_player_loadercolor', $ap_color, "Loader bar color", true);
	add_ap_option('audio_player_bordercolor', $ap_color, "Border color", true);
	delete_option('audio_player_barcolor');

	add_ap_option('audio_player_slidercolor', '0x666666', "Progress slider color", true);
} else {
	// Default color options
	add_ap_option('audio_player_bgcolor', '0xf8f8f8', "Background color", true);
	add_ap_option('audio_player_textcolor', '0x666666', "Text color", true);
	add_ap_option('audio_player_leftbgcolor', '0xeeeeee', "Left background color", true);
	add_ap_option('audio_player_lefticoncolor', '0x666666', "Left icon color", true);
	add_ap_option('audio_player_rightbgcolor', '0xcccccc', "Right background color", true);
	add_ap_option('audio_player_rightbghovercolor', '0x999999', "Right background hover color", true);
	add_ap_option('audio_player_righticoncolor', '0x666666', "Right icon color", true);
	add_ap_option('audio_player_righticonhovercolor', '0xffffff', "Right icon hover color", true);
	add_ap_option('audio_player_slidercolor', '0x666666', "Progress slider color", true);
	add_ap_option('audio_player_trackcolor', '0xFFFFFF', "Progress track color", true);
	add_ap_option('audio_player_loadercolor', '0x9FFFB8', "Loader bar color", true);
	add_ap_option('audio_player_bordercolor', '0x666666', "Border color", true);
}
...
$ap_playerURL = get_ap_option('siteurl') . '/wp-content/plugins/audio-player/player.swf';
$ap_audioURL = get_ap_option('siteurl') . get_ap_option("audio_player_web_path");
// Initialise playerID (each instance gets unique ID)
$ap_playerID = 0;
// Convert behaviour options to array
$ap_behaviour = explode( ",", get_ap_option("audio_player_behaviour") );
...
function ap_set_options() {
	...
	foreach( $ap_colorkeys as $value ) $ap_options[$value] = get_ap_option("audio_player_" . $value . "color");
}
...
function ap_insert_player_widgets($content = '') {
	...
	if( in_array( "enclosure", $ap_behaviour ) ) {
		...
		$prefixAudio = get_ap_option( "audio_player_prefixaudio" );
		if( $prefixAudio != "" ) $prefixAudio .= ",";
		$postfixAudio = get_ap_option( "audio_player_postfixaudio" );
...
function ap_getplayer($source, $options = array()) {
	...
	if(is_feed()) {
		...
		switch( get_ap_option( "audio_player_rssalternate" ) ) {
			case "download":
			...
			case "nothing":
				...
				break;
			case "custom":
				return get_ap_option( "audio_player_rsscustomalternate" );
				break;
		}
	}
	// Not in a feed so return formatted object tag
	else return '<object type="application/x-shockwave-flash" data="' . $ap_playerURL . '" width="290" height="24" id="audioplayer' . $ap_playerID . '"><param name="movie" value="' . $ap_playerURL . '" /><param name="FlashVars" value="' . $flashVars . '" /><param name="quality" value="high" /><param name="menu" value="false" /><param name="wmode" value="transparent" /></object>';
}
...
function ap_options_subpanel() {
	...
	if( $_POST['ap_updateCheck'] ) {
		...
		foreach ( $_POST as $key => $val )
		{
			if ( !is_array($val) )
			{
				$_POST[$key] = wp_filter_post_kses($val);
			}
			else
			{
				foreach ( $_POST[$key] as $key2 => $val2 )
				{
					$_POST[$key][$key2] = wp_filter_post_kses($val2);
				}
			}
		}
		...
		update_ap_option('audio_player_web_path', $_POST['ap_audiowebpath']);

		// Update behaviour and rss alternate content options
		update_ap_option('audio_player_behaviour', implode(",", $_POST['ap_behaviour']));
		update_ap_option('audio_player_rssalternate', $_POST['ap_rssalternate']);
		update_ap_option('audio_player_rsscustomalternate', $_POST['ap_rsscustomalternate']);
		update_ap_option('audio_player_prefixaudio', $_POST['ap_audioprefixwebpath']);
		update_ap_option('audio_player_postfixaudio', $_POST['ap_audiopostfixwebpath']);

		$default_colors = array(
			'audio_player_bgcolor' => '0xf8f8f8',
			'audio_player_textcolor' => '0x666666',
			'audio_player_leftbgcolor' => '0xeeeeee',
			'audio_player_lefticoncolor' => '0x666666',
			'audio_player_rightbgcolor' => '0xcccccc',
			'audio_player_rightbghovercolor' => '0x999999',
			'audio_player_righticoncolor' => '0x666666',
			'audio_player_righticonhovercolor' => '0xffffff',
			'audio_player_slidercolor' => '0x666666',
			'audio_player_trackcolor' => '0xFFFFFF',
			'audio_player_loadercolor' => '0x9FFFB8',
			'audio_player_bordercolor' => '0x666666',
			);
		...
		foreach( $ap_colorkeys as $colorkey ) {
			...
			if( isset( $_POST["ap_" . $colorkey . "color"] ) && preg_match( "/#[0-9A-Fa-f]{6}/", $_POST["ap_" . $colorkey . "color"] ) == 1 ) {
				update_ap_option( "audio_player_" . $colorkey . "color", str_replace( "#", "0x", $_POST["ap_" . $colorkey . "color"] ) );
			}
			else
			{
				update_ap_option( "audio_player_" . $colorkey . "color", $default_colors["audio_player_" . $colorkey . "color"] );
			}
		}
	...
	$ap_behaviour = explode(",", get_ap_option("audio_player_behaviour"));
	$ap_rssalternate = get_ap_option('audio_player_rssalternate');
	...
	include( dirname(__FILE__) . "/audio-player/options-panel.php" );
}

// Add options page to admin menu
function ap_post_add_ap_options() {
...
add_action('admin_menu', 'ap_post_add_ap_options');

// Output script tag in WP front-end head
function ap_wp_head() {
	global $ap_playerID;
	echo '<script language="javascript1.4" type="text/javascript" src="' . get_ap_option("siteurl") . '/wp-content/plugins/audio-player/audio-player.js"></script>';
	echo "\n";
}
...
function ap_wp_admin_head() {
	global $ap_playerID;
	echo '<script language="javascript" type="text/javascript" src="' . get_ap_option("siteurl") . '/wp-content/plugins/audio-player/audio-player-admin.js"></script>';
	echo "\n";
	echo '<script language="javascript" type="text/javascript">';
	echo "\n";
	echo 'var ap_updateURL = "' . get_ap_option("siteurl") . '/wp-content/plugins/audio-player/upgrade/";';
	echo "\n";
	echo '</script>';
	echo "\n";
}
