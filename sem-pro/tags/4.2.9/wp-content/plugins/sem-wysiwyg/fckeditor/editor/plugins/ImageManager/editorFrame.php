...
<script type="text/javascript">
	...
	var currentImageFile = "<?php if(count($imageInfo)>0) echo $imageInfo['file']; ?>";
	if(window.top)
	{
...
?>
	...
	// save message
	var message = "<?php echo $editor->fileSavedName; ?>" + i18n(" saved.");
<?php
...
<?php
	...
	{
?>
	alert("<?php echo $editor->fileSavedName; ?>" + i18n("File was not saved."));
<?php
	}
	else
?>
...
<?php if(count($imageInfo) > 0 && is_file($imageInfo['fullpath'])) { ?>
		...
		<div id="background" style="display: none; margin: auto; width: <?php echo $imageInfo['width']; ?>px; height: <?php echo $imageInfo['height']; ?>px; background-image: url(<?php echo rawurldecode($imageInfo['src']); ?>);">
		<?php if (count($IMConfig['watermarks']) > 0) { ?>
			<img id="floater" style="width: 150px; height: 150px; behavior: url('assets/pngbehavior.htc'); position: absolute" src="<?php echo $IMConfig['base_url'] . $IMConfig['watermarks'][0]; ?>" />
		<?php } ?>
		</div>
