...
function drawFiles($list, &$manager)
{
	...
	<a href="javascript:;" onclick="selectImage('<?php echo Files::escape_quote($file['relative']);?>', '<?php echo Files::escape_quote($entry); ?>', <?php echo $file['image'][0];?>, <?php echo $file['image'][1]; ?>);"title="<?php echo $entry; ?> - <?php echo Files::formatSize($file['stat']['size']); ?>"><img src="<?php echo $manager->getThumbnail($file['relative']); ?>" alt="<?php echo $entry; ?> - <?php echo Files::formatSize($file['stat']['size']); ?>"/></a>
		...
		<?php if($IMConfig['allow_delete'] == true) { ?>
			<a href="images.php?dir=<?php echo $relative; ?>&amp;delf=<?php echo rawurlencode($file['relative']);?>" title="Trash" onclick="return confirmDeleteFile('<?php echo Files::escape_quote($entry); ?>');"><img src="img/edit_trash.gif" height="15" width="15" alt="Trash"/></a>
		<?php } ?>
...
function drawDirs($list, &$manager)
{
	...
	{ ?>
		...
		<td class="edit">
			<a href="images.php?dir=<?php echo $relative; ?>&amp;deld=<?php echo rawurlencode($path); ?>" title="Trash" onclick="return confirmDeleteDir('<?php echo Files::escape_quote($dir['entry']); ?>', <?php echo $dir['count']; ?>);"><img src="img/edit_trash.gif" height="15" width="15" alt="Trash"/></a>
			<?php echo $dir['entry']; ?>
		</td>
		</tr></table></td>
	  <?php
