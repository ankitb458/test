...
class ImageManager 
{
		...
		else
		{
			...
			switch($result) {
				case FILE_ERROR_NO_SOURCE:
					...
					break;
				case FILE_ERROR_COPY_FAILED:
					errorMessage("An error occurred copying the uploaded file to the $path directory.");
					break;
				case FILE_ERROR_DST_DIR_FAILED:
					errorMessage("The $path directory does not exist.");
					break;
				case FILE_ERROR_DST_DIR_NOT_WRITABLE:
	...
	function processNewDir() 
	{
		...
		{
			$newDir = Files::unescape_quote(rawurldecode($_GET['newDir']));
			$dir = rawurldecode($_GET['dir']);
			$path = Files::makePath($this->getBaseDir(),$dir);
			$fullpath = Files::makePath($path, $newDir);
			if(is_dir($fullpath))
				Return false;
