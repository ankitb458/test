...
<div id="tools">
	<div id="tools_replace" style="display:none;">
		...
		<tr>
			<td>
				<form action="editorFrame.php?img=<?php echo Files::unescape_quote($_GET['img']);?>&amp;action=replace" target='editor' id="uploadForm" method="post" enctype="multipart/form-data">
					&nbsp;<input type="file" name="upload" id="upload"/>
					<input type="hidden" name="dir" id="dir" value="<?php echo dirname(Files::unescape_quote($_GET['img']))?>" />
				</form>
			</td>
			<td>
				...
				<table style="display: inline; vertical-align: bottom; margin: 0px 5px 0px 20px;" cellpadding="0" cellspacing="0">
					...
					<td>
						<div style="cursor: pointer; cursor: hand; background-image: url(img/watermarkAlign.gif); vertical-align: middle; width: 24px; height: 24px; position: relative;">
							...
							<div style="position: absolute; left: 1px; top: 10px; width: 4px; height: 4px; overflow: hidden;" onmouseover="this.style.backgroundColor='#BE3545'" onmouseout="this.style.backgroundColor='transparent'" onclick="moveWatermark(0, 0.5);"></div>
	...
	<div id="tools_rotate" style="display:none;">
		<div id="tool_inputs">
			<select id="flip" name="flip" style="margin-left: 10px; vertical-align: middle;">
              ...
              <option>-----------------</option>
              <option value="hoz">Flip Horizontally</option>
              <option value="ver">Flip Vertically</option>
         </select>
	...
	<div id="tools_measure" style="display:none;">
		<div id="tool_inputs">
			...
			<label id="yLabel">Y:<input type="text" class="measureStats" id="sy" disabled="disabled" /></label>
			<img src="img/div.gif" height="30" width="2" class="div" alt="|" />
			<label id="widthLabel">W:<input type="text" class="measureStats" id="mw" disabled="disabled" /></label>
			<label id="heightLabel">H:<input type="text" class="measureStats" id="mh" disabled="disabled" /></label>
			<img src="img/div.gif" height="30" width="2" class="div" alt="|" />
			<label id="angleLabel">A:<input type="text" class="measureStats" id="ma" disabled="disabled" /></label>
			<label id="diagonalLabel">D:<input type="text" class="measureStats" id="md" disabled="disabled" /></label>
			<img src="img/div.gif" height="30" width="2" class="div" alt="|" />
...
<?php if($IMConfig['demo'] != true) { ?>
	...
	<?php if($IMConfig['allow_newFileName'] == true) { ?>
			<label for="save_filename">Filename:</label><input type="text" id="save_filename" value="<?php if($IMConfig['allow_overwrite'] == false){ echo $editor->getDefaultSaveFile(); }else{ echo basename(Files::unescape_quote($_GET['img'])); } ?>" />
	<?php }else{ ?>
			<input type="hidden" id="save_filename" value="<?php echo basename(Files::unescape_quote($_GET['img'])); ?>" />
	<?php } ?>

	<?php
	$pos = strrpos(Files::unescape_quote($_GET['img']), ".");
	$ext = substr(Files::unescape_quote($_GET['img']), $pos + 1);
	?>
...
<iframe src="editorFrame.php?img=<?php if(isset($_GET['img'])) echo rawurlencode(Files::unescape_quote($_GET['img'])); ?>" name="editor" id="editor" scrolling="auto" title="Image Editor" frameborder="0"></iframe>
</div>
<div id="bottom"></div>
</body>
</html>
