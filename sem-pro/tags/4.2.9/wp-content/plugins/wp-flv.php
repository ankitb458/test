...
function update_vp_option($name, $value)
{
	if ( function_exists('get_site_option') )
	{
...
EOT;
	...
	function wpflv_get_options(){
		...
		$defaults['videourl'] = '/movies';
		$defaults['width'] = '320';
		$defaults['height'] = '240';
		$defaults['autostart'] = 'n';
		$defaults['xhtmlvalid'] = 'n';

		$options = get_vp_option('rmnlFLVsettings');
		if (!is_array($options)){
			$options = $defaults;
			update_vp_option('rmnlFLVsettings', $options);
		}

		$options['playerurl'] = trailingslashit(get_settings('siteurl')) . 'video/flvplayer.swf';
	...
	function wpflv_options_page(){
		if ($_POST['wpflv']){
			...
			update_vp_option('rmnlFLVsettings', $_POST['wpflv']);
...
<?php endif; ?>			<fieldset class="options">
			...
			if($o['quicktags'] == 'y'){
				...
				<script type="text/javascript">
					<!--
						...
						if(flvToolbar){
							var flvBut = new edButton('ed_flv','','','','');
...
//								var A = confirm('Do you want this video to start playing automatically?');
								var theTag = '<flv href="' + U + '" width="' + W + '" height="' + H + '" />';
								//edButtons[flvNr].tagStart = theTag;
								//edInsertTag(edCanvas, flvNr);
								edInsertContent(edCanvas, theTag);
//							} else {
//								edInsertTag(edCanvas, flvNr);
//							}
						}
