...
Plugin URI: http://www.semiologic.com/software/widgets/recent-posts/
Description: A WordPress widget that lists a fuzzy number of recently posted entries.
...
class sem_recent_posts
{
	...
	function flush_cache()
	{
		...
		{
			$cache_files = glob(sem_cache_path . "sem-recent-posts*");
	...
	{
		add_options_page(
				__('Fuzzy&nbsp;Posts', 'sem-recent-posts'),
				__('Fuzzy&nbsp;Posts', 'sem-recent-posts'),
				'manage_options',
	...
	function update()
	{
		check_admin_referer('fuzzy_posts');
	...
	function display_admin_page()
	{
		...
		if ( function_exists('wp_nonce_field') ) wp_nonce_field('fuzzy_posts');
	...
	function display($args = null)
	{
		...
		{
			$params['main_cat_id'] = $params['cat_id'];
		}
	...
	function widgetize()
	{
		...
		{
			register_sidebar_widget('Fuzzy Posts', array(&$this, 'display_widget'));
			register_widget_control('Fuzzy Posts', array(&$this, 'widget_control'));
		}
	} # end widgetize()
	...
	{
		$options = get_settings('sem_recent_posts_params');

		if ( $_POST["sem_recent_posts_widget_update"] )
		{
			$new_options = $options;

			$new_options['title'] = stripslashes(wp_filter_post_kses(strip_tags($_POST["sem_recent_posts_widget_title"])));

			if ( $options != $new_options )
			{
				$options = $new_options;

				update_option('sem_recent_posts_params', $options);
			}
		}
		...
		echo '<input type="hidden"'
				. ' id="sem_recent_posts_widget_update"'
				. ' name="sem_recent_posts_widget_update"'
				. ' value="1"'
				. ' />'
			. '<p>'
			. '<label for="sem_recent_posts_widget_title">'
				. __('Title:')
				. '&nbsp;'
				. '<input style="width: 250px;"'
					. ' id="sem_recent_posts_widget_title"'
					. ' name="sem_recent_posts_widget_title"'
