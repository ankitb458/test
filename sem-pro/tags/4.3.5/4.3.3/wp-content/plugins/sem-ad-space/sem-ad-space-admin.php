...
class sem_ad_space
{
	...
	function sem_ad_space()
	{
		...
		if ( !isset($this->params['version']) || $this->params['version'] < $this->version )
		{
			$this->upgrade();
		}
	...
	function upgrade()
	{
		...
		{
			$duplets = (array) $wpdb->get_results("
				SELECT post_id, ad_distribution_id
				FROM $wpdb->ad_distribution2post
				");

			foreach ( $duplets as $duplet )
			{
				delete_post_meta($duplet->post_id, '_ad_distribution');

				$value = $duplet->ad_distribution_id;

				if ( $value !== '' )
				{
					add_post_meta($duplet->post_id, '_ad_distribution', $value, true);
				}

				$wpdb->query("
					DELETE FROM $wpdb->ad_distribution2post
					WHERE post_id = $duplet->post_id
					");
			}
